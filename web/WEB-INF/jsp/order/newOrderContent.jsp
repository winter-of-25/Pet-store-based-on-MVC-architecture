...
<div class="wrap">
    ...
    <div id="Checkout">
        <div id="CheckoutContent">
            ...
            <div id="newOrderForm">
                <p>收货人：<%= (name != null ? name : "未选择") %></p>
                <p>电话：<%= (phone != null ? phone : "未选择") %></p>
                <p>地址：<%= (address != null ? address : "未选择") %></p>
            </div>

            <div id="orderSummary" style="margin-top: 20px;">
                <p>支付方式：<%= (payment != null ? payment : "未选择") %></p>
            </div>

            <form action="checkoutForm" method="post" onsubmit="logEvent('CHECKOUT_SUBMIT','提交结算')">
...
<script>
    // 页面加载时，从 session 读取值填充输入框
    $(function(){
        $('#receiverName').val('<%= session.getAttribute("name") != null ? session.getAttribute("name") : "" %>');
        $('#receiverPhone').val('<%= session.getAttribute("phone") != null ? session.getAttribute("phone") : "" %>');
        $('#receiverAddress').val('<%= session.getAttribute("address") != null ? session.getAttribute("address") : "" %>');
